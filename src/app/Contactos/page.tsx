...
export default function Contactos() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const contactsRes = await fetch(`${BASE_URL}/api/contacts`, { signal: controller.signal });
        const kanbanRes = await fetch(`${BASE_URL}/kanban/${ownerId}`, { signal: controller.signal });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-8 relative">
      <div className={`fixed top-4 right-4 z-50 transition-opacity duration-300 ${toast.visible ? 'opacity-100' : 'opacity-0'}`}>
        <div className={`${toast.type === 'success' ? 'bg-green-50 border-green-400' : 'bg-red-50 border-red-400'} p-4 rounded-lg border-l-4 shadow-lg flex items-center gap-3 min-w-[300px]`}>
          {toast.type === 'success' ? (
          ...
          <span className={`${toast.type === 'success' ? 'text-green-700' : 'text-red-700'} font-medium`}>{toast.message}</span>
        </div>
      </div>

      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-800 mb-8 text-center">📚 Directorio de Contactos</h1>
        {contacts.length === 0 ? (
